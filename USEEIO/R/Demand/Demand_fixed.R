...
BEApath <- file.path(ifelse(dir.exists('Z:/'), 'Z:', '/nfs/fwe-data'), 'BEA/formatted')
...
getUSTotalConsProd = function (startyear,endyear,CPIreferenceyear,imputedemand=FALSE,usetablefile, columns_modify = character(0), rows_modify = character(0), row_factor = numeric(0)) {
  ...
  if(imputedemand) {
	...
	demand_detail <- getUSDetailDemandfromUseTable(year=startyear, usetablefile=usetablefile, columns_modify = columns_modify, rows_modify = rows_modify, R_row = row_factor)
    temp[[startyear]] = as.data.frame(cbind(rowSums(demand_detail[,TtlCons]),
                                                   rowSums(demand_detail[,TtlProd])))
    colnames(temp[[startyear]]) = paste0(startyear, c("_US_Consumption","_US_Production"))
    USConsProd = temp[[startyear]]
  }
  ...
  CPI = read.table(file.path(BEApath,"389_CPI.csv"),sep = ",",header=T,row.names=1,check.names=F) 
