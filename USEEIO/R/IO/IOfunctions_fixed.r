...
generateMarketShareCoefficientfromMake = function(maketablefile, year=2007, remove_scrap=TRUE) {
  IO_Make_389 = read.table(maketablefile,header=T,sep = ",",row.names=1,check.names=F)[,-390]
  IO_Make_389 = as.matrix(IO_Make_389)
  ms = sweep(IO_Make_389, 2, IO_Make_389['T007', ], '/')
  rownames(ms) = rownames(read.table(maketablefile,header=T,sep = ",",row.names=1,check.names=F))
...
generateNonScrapRatiosfromMake = function(maketablefile, year=2007) {
  
  IO_Make_389 = read.table(maketablefile,header=T,sep = ",",row.names=1,check.names=F)[-390,]
  IO_Make_389 = apply(IO_Make_389, 2, as.numeric)
  IO_Make_389[is.na(IO_Make_389)] = 0
  ratios = data.frame((IO_Make_389[,"T008"]-IO_Make_389[,"S00401"])/IO_Make_389[,"T008"])
  rownames(ratios) = rownames(read.table(maketablefile,header=T,sep = ",",row.names=1,check.names=F))[1:389]
  return(ratios)
}

generateTransformationMatrixfromMake = function(maketablefile, year=2007) {
  ms = data.frame(generateMarketShareCoefficientfromMake(maketablefile, year),check.names = FALSE)
  nonscrapratios = as.matrix(generateNonScrapRatiosfromMake(maketablefile, year))
...
generateDirectRequirementCoefficientfromUse = function (usetablefile, remove_scrap=TRUE) {
  IO_Use_389 = read.table(usetablefile,header=T,sep = ",",row.names=1,check.names=F)[,1:389]
  IO_Use_389 = as.matrix(IO_Use_389)
  dr = sweep(IO_Use_389, 2, IO_Use_389['T008',], '/')
  rownames(dr) = rownames(read.table(usetablefile,header=T,sep = ",",row.names=1,check.names=F))
  if(remove_scrap==TRUE) {
...
formatIOTableforIOMB = function (IOtable,state_model=FALSE,majormodelversion=1) {
...
generateDR1RegionCoeffs = function (year = Y, usetablefile, maketablefile) {
  dr = generateDirectRequirementCoefficientfromUse(usetablefile = usetablefile)
  tm = generateTransformationMatrixfromMake(maketablefile, year)
